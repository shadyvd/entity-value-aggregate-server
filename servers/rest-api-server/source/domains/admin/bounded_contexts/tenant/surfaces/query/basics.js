...
export class Basics extends BaseSurface {
	...
	async _registerSurface() {
		...
		baseRoutes?.push?.({
			...
			path: '/all',
			middlewares: [
				await this?._rbac?.('admin-tenant-basics-read'),
				await this?._abac?.()
			],
			handler: this.#getAllTenants?.bind?.(this)
		});
		...
		baseRoutes?.push?.({
			...
			handler: this.#getTenant?.bind?.(this)
		});
	...
	async #getAllTenants(ctxt) {
		const tenant = await this?.domainInterface?.apiRegistry?.execute?.(
			'TENANT_BASICS::GETALL',
			{
		...
		ctxt.status = tenant?.status;
		ctxt.body = tenant?.body;
	}

	async #getTenant(ctxt) {
		const tenant = await this?.domainInterface?.apiRegistry?.execute?.(
			'TENANT_BASICS::GET',
			{
				tenant: ctxt?.state?.tenant,
				user: ctxt?.state?.user,
				tenantId:
					ctxt?.request?.params?.tenantId ?? ctxt?.state?.tenant?.id
			}
		);
